# Задача 08. Тесты `TransitionGuard` (Этап A)

## Цель
Покрыть `TransitionGuard.attempt` юнит-тестами: все 5 веток логики (force/self/illegal/artifact/
allowed) + боковые переходы + граничные. По стилю `TaskStateMachineTest`.

## Зависимости
07 (`TransitionGuard`). Образец: `src/test/kotlin/com/cliagent/state/TaskStateMachineTest.kt`.

## Тест-класс (новый)
`src/test/kotlin/com/cliagent/state/TransitionGuardTest.kt`
...
class TransitionGuardTest {
    ...
    @Test
    fun `illegal outcome does not change state`() {
        val state = TaskState(stage = TaskStage.PLANNING, approvedPlan = "plan")
        val outcome = TransitionGuard.attempt(state, TaskStage.DONE)
        // Illegal не несёт newState — состояние потребителя не меняется
        assertTrue(outcome is TransitionOutcome.Illegal)
    }
...
|---|---|---|---|---|---|
| 1 | PLANNING | EXECUTION | plan | false | Allowed |
| 2 | CLARIFY | PLANNING | — | false | Allowed (нет gate) |
| 3 | VALIDATION | EXECUTION | — | false | Allowed (боковой) |
| 4 | EXECUTION | PLANNING | — | false | Allowed (боковой) |
| 5 | PLANNING | DONE | plan | false | Illegal (перепрыг) |
| 6 | CLARIFY | EXECUTION | — | false | Illegal (перепрыг) |
| 7 | PLANNING | EXECUTION | пусто | false | ArtifactMissing |
| 8 | VALIDATION | DONE | пусто | false | ArtifactMissing |
| 9 | PLANNING | DONE | — | true | Allowed (force, note="forced") |
| 10 | PLANNING | EXECUTION | пусто | true | Allowed (force обходит gate) |
| 11 | PLANNING | PLANNING | — | false | Allowed (self, no history) |

## Критерии готовности
- `./gradlew test --tests "*TransitionGuardTest"` зелёный.
- ~17 тестов покрывают все 5 веток + боковые + force + self.
- `Illegal.allowedTargets` корректен (содержит self + разрешённые).
- `ArtifactMissing.hint` содержит человекочитаемую команду (`/task plan`/`impl`/`verdict`).
- Force-mode дописывает note="forced".
