...
class ExecutionStageAgent(
    ...
    /** Инструкция «напрямую без плана» по типу задачи (фикс #1; день 34 — FILE_OP/PR_REVIEW). */
    private fun directInstruction(taskKind: TaskKind?): String = when (taskKind) {
        ...
        TaskKind.FILE_OP -> "Выполни задачу напрямую через file-тулы: read_file/find_in_files для анализа, write_file для изменений (с подтверждением)."
        TaskKind.PR_REVIEW -> "Проанализируй изменения напрямую: используй git_diff/read_file, дай review-отчёт."
