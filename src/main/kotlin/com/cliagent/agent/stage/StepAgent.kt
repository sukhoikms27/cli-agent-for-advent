...
class StepAgent {
    ...
    /** Финальная инструкция шага по типу задачи (фикс #1; день 34 — FILE_OP/PR_REVIEW). */
    private fun stepInstruction(taskKind: TaskKind?): String = when (taskKind) {
        ...
        TaskKind.FILE_OP ->
            "Выполни текущий шаг через file-тулы: read_file/find_in_files/list_project_files для " +
                "анализа, write_file для изменений (требует подтверждения). Опирайся на реально " +
                "прочитанное содержимое, не выдумывай структуру."
        TaskKind.PR_REVIEW ->
            "Выполни текущий шаг: проанализируй фрагмент изменений (через git_diff/read_file), " +
                "дай замечания и рекомендации. Опирайся на реально прочитанный код."
        null ->
